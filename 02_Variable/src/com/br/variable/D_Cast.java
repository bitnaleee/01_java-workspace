...
public class D_Cast {
	 ...
	 *    혹시라도 다른자료형의 값을 대입하고자 한다면 형변환이 필수
	 *    
	...
	public void autoCasting() {
		...
		/*
		 * * 자동 형변환
		 *   자료형이 다른 두 값 간의 연산(대입, 계산 등등) 시
		 *   자동으로 값의 범위가 작은 자료형이 큰 자료형으로 변환되어 처리됨
		 */
		
		// 1. int(4byte) --> double(8byte)
		int i1 = 12;
		double d1 = /*(double)*/i1; //자동형변환 12 => 12.0
		...
		double result = /*(double)*/i2 + d2;	//자료형이 맞아야 되는데 맞지 않는데도 오류가 안뜨는 이유는 자동형변환이 되고 있기 때문 int->double
												//12.0 + 3.3 => 15.3
		...
		float f4 = 1.0f;	//f 안붙이면 double로 인식 8byte
		double d4 = /*(double)*/f4;
				
		//=============== 특이케이스 =================
		// 특이1. long(8byte) --> float(4byte)
		//       float이 실수형이기 때문에 long형보다도 표현 가능한 수의 범위가 더 크기 때문에
				
		float f5 = /*(float)*/ l3;
		
		// 특이2. char(2byte) <--> int(4byte)
		int num = 'A';
		char ch = 53210;	// 단, char에 음수값 저장 불가 양수만 가능 => 0 ~ 65535
	...
	public void forceCasting() {
		// 강제형변환 : 큰 크기의 자료형을 작은 크기의 자료형으로 강제로 바꾸는 거
		...
		//int result = iNum + dNum; // 10.0 + 5.89 => 15.89 덧셈연산할 때는 자동형변환으로 문제x 결과가 int 자료형에 대입할 때 문제 
								  // 연산결과인 double형이 int형에 대입 불가
		
		// 해결방법1. 연산결과를 int형으로 강제형변환 후 대입
		int result1 = (int)(iNum + dNum);	// (int)15.89 => 15 소수점 아래가 버려지기 때문에 약간의 데이터 손상
