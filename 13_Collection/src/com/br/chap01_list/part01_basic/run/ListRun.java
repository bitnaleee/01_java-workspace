...
public class ListRun {
		 ...
		 * 
		 * 	  > 배열
		 * 		- 크기에 대한 제약이 많음 (크기 지정 필수, 한번 지정되 크기 변경 불가) 
		 * 		- 중간 위치에 추가하거나 삭제하는 경우 값을 뒤로 또는 앞으로 땡겨주는 작업을 직접 코드로써 구현해야됨
		 * 		- 한 타입의 데이터만 저장가능
		 * 
		 * 
		 * 	  > 컬렉션
		 * 		- 크기에 대한 제약이 없음 (크기 지정 안해도 됨, 알아서 크기 변경됨)
	...
	public static void main(String[] args) {
		...
		System.out.println("리스트의 사이즈 : " + list.size());	 // 배열과 비교하면 length같은 느낌 
		System.out.println("리스트의 마지막 인덱스수 : " + (list.size() -1));
		
		// 6. get(int index) : E	=> 해당 인덱스위치의 객체를 반환시켜주는 메소드 // 배열이었다면 list[0]
		Music m = (Music)list.get(0);	// 강제형변환
		String s= (String)list.get(3);	// 강제형변환
