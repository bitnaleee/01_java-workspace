...
public class HashSetRun {
	...
	public static void main(String[] args) {
		...
		// 새로운 데이터를 추가할 때마다 동일객체(hasCode값이 일치한지, equals비교시 true인지)인지 판단함
		
		System.out.println(hs1);	// 순서유지 안됨, 중복된 데이터(동일객체) 보관 불가능
		// hash코드가 다르면 동일객체가 아니구나 인식 equals검사안함
		...
		System.out.println(hs2);	// 중복 제거 x => 동일객체로 판단이 안되고 있기 때문
		
		// 동일객체 : 각 객체마다 hashCode결과가 일치, equals 비교시 true여야됨
		
		// Student에 equals() 오버라이딩 => "실제 각 필드에 담긴 데이터"들이 다 일치하면 true / 그게 아니면 false
		// Studnet에 hashCode() 오버라이딩 => "실제 각 필드에 담긴 데이터"들이 일치하면 동일한 10진수 반환
		...
		// >> ArrayList에 옮겨담기 2. ArrayList 생성시 통래로 추가하기
		...
		while(it.hasNext()) {	//it공간으로부터뽑을객체가있을경우
			Student s = it.next();
			System.out.println(s);
		}
		
		// StringTokernizer와 비슷한 개념 //it.next(); // NoSuchElementException 발생
