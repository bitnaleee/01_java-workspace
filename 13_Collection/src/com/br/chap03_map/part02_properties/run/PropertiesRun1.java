...
public class PropertiesRun1 {
	...
	public static void main(String[] args) {
		...
		// 단, Properties를 주로 사용하는 경우는 Properties에 담겨있는 것들을
		// 파일로 출력 또는 입력받아 올 때 사용함
		// store() : 파일로 저장 (출력)
		// load() : 파일로부터 불러오기 (입력)
		
		try {
			prop.store(new FileOutputStream("test.properties"), "propertiesTest");
			// 내부적으로 Properties에 담겨있는 키, 벨류 들을 String 형변환해서 출력함
		} catch (IOException e) {
			e.printStackTrace();
		}
		*/
		...
		System.out.println(prop.getProperty("list"));	// 존재하지 않은 키값 제시하면 null반환**
		
		
		try {
			// 3. store(OutputStream os, String comments) => Properties안에 담겨있는 key-value 세트들을 파일로 출력
			prop.store(new FileOutputStream("test.properties"), "propertiesTest");
